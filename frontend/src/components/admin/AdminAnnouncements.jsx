...
const AdminAnnouncements = () => {
  ...
  const fetchNotifications = async () => {
    try {
      // Backend now filters by adminId for admins
  ...
  const fetchUserStats = async () => {
    try {
      ...
      setUsers({
        ...
        guards: allUsers.filter(u => u.role === 'guard').length,
  ...
  const handleSend = async () => {
    ...
    try {
      // Call the broadcast API to send announcement to all users
      ...
      const announcement = {
        ...
        isRead: false,
        recipientCount: newAnnouncement.target === 'all' ? users.total : 
                       newAnnouncement.target === 'tenants' ? users.tenants : users.guards
      };
    ...
    <div style={containerStyle}>
      ...
      >
        ...
        <p style={{
          ...
          Broadcast messages to tenants, guards, or all users
        </p>
      </motion.div>
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            <button
              ...
              style={{
                padding: '12px 24px',
                borderRadius: '12px',
                border: `2px solid ${newAnnouncement.target === 'all' ? '#3B82F6' : '#334155'}`,
                background: newAnnouncement.target === 'all' ? '#3B82F620' : 'transparent',
                color: '#F1F5F9',
                fontWeight: 600,
                cursor: 'pointer',
                display: 'flex',
            ...
            <button
              onClick={() => setNewAnnouncement({ ...newAnnouncement, target: 'tenants' })}
              style={{
                padding: '12px 24px',
                borderRadius: '12px',
                border: `2px solid ${newAnnouncement.target === 'tenants' ? '#22C55E' : '#334155'}`,
                background: newAnnouncement.target === 'tenants' ? '#22C55E20' : 'transparent',
                color: '#F1F5F9',
                fontWeight: 600,
                cursor: 'pointer',
                display: 'flex',
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                      ...
                      <span style={{
                        ...
                        fontSize: '11px',
                        fontWeight: 600,
                        background: announcement.target === 'all' ? '#3B82F620' :
                                   announcement.target === 'tenants' ? '#22C55E20' : '#F59E0B20',
                        color: announcement.target === 'all' ? '#3B82F6' :
                               announcement.target === 'tenants' ? '#22C55E' : '#F59E0B',
